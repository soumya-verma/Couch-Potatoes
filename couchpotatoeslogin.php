...
<head>
	...
	<title>Couch Potatoes Login</title>
</head>
<?php
...
<script type="text/javascript">
	function validate1()
	{
		...
		//{
			if(x.match(/[a-z]/g) && x.match(/[A-Z]/g) && x.match(/[0-9]/g) && x.match(/[^!@#$%^&*()\:.,/?]/g))
			{
				if(y.match(/[a-z]/g) && y.match(/[A-Z]/g) && y.match(/[0-9]/g) && y.length>=6 && y.length<=20)
					return true;
				else
		...
		<div name="a2" class="hello">
			<div name="a3" class="div">
				<form class="form" name="myform" method="post" action="couchpotatoeslogin.php">
					...
					<div class="login">
						<button class="loginbut" onclick="return validate1()">Login</button>
					</div>
					<p class="new">Are you a new user? <button class="signupbtn" formaction="couchpotatoessignup.html">SIGNUP</button></p>
				</form>
			</div>
	    </div>
	</div>
